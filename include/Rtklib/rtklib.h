...
#define rtklib_pi 3.1415926535897932 /* rtklib_pi */
#define D2R (rtklib_pi / 180.0)      /* deg to rad */
#define R2D (180.0 / rtklib_pi)      /* rad to deg */
...
#define MAXRTKSAT (NSATGPS + NSATGLO + NSATGAL + NSATQZS + NSATCMP + NSATIRN + NSATSBS + NSATLEO)
    /* max satellite number (1 to MAXRTK) */
#define MAXSTA 255
...
#define FILEPATHSEP '/'
    ...
    {                             /* SSR correction type */
        ...
        double ifpb;              /* high-rate ifpb correction (m) */
        ...
        gtime_t ifpb_t0;          /* ifpb reference time */
    ...
    {                                              /* RTCM control struct type */
        ...
        uint16_t lock[MAXRTKSAT][NFREQ + NEXOBS];  /* lock time */
        uint16_t loss[MAXRTKSAT][NFREQ + NEXOBS];  /* loss of lock count */
    ...
    {                      /* processing options type */
        ...
        uint8_t exsats[MAXRTKSAT]; /* excluded satellites (1:excluded,2:included) */
    ...
    {                                 /* RINEX options type */
        ...
        uint8_t exsats[MAXRTKSAT];    /* excluded satellites */
    ...
    {                                                 /* receiver raw data control type */
        ...
        char msgtype[256];                            /* last message type */
        uint8_t subfrm[MAXRTKSAT][380];               /* subframe buffer */
        ...
        uint8_t halfc[MAXRTKSAT][NFREQ + NEXOBS];     /* half-cycle add flag */
